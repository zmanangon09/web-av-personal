...
function AppMailTable(props: AppMailTableProps) {
    ...
    const handleArchive = (event: React.MouseEvent<HTMLSpanElement, MouseEvent> | React.TouchEvent<HTMLSpanElement>, mail: Demo.Mail) => {
        event.stopPropagation();
            ...
            <div className="cursor-pointer" style={{ minWidth: '10rem' }}>
                <div className="flex justify-content-end w-full px-0">
                    ...
                    <div id={columnOptions.rowIndex.toString() + '-options'} style={{ display: 'none' }}>
                        <Button type="button" tooltip="Archive" tooltipOptions={{ position: 'top' }} icon="pi pi-inbox" className="h-2rem w-2rem mr-2" onClick={(event) => handleArchive(event, mail)}></Button>
